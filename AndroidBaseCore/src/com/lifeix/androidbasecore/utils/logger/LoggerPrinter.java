...
final class LoggerPrinter implements Printer {
	...
	private void logChunk(int logType, String tag, String chunk) {
		...
		case Log.ERROR:
			...
			if (settings.isLogger2File()) {
				settings.getLogger2File().e(finalTag, chunk);
			}
			break;
		case Log.INFO:
			Log.i(finalTag, chunk);
			if (settings.isLogger2File()) {
				settings.getLogger2File().i(finalTag, chunk);
			}
			break;
		case Log.VERBOSE:
			Log.v(finalTag, chunk);
			if (settings.isLogger2File()) {
				settings.getLogger2File().v(finalTag, chunk);
			}
			break;
		case Log.WARN:
			Log.w(finalTag, chunk);
			if (settings.isLogger2File()) {
				settings.getLogger2File().w(finalTag, chunk);
			}
			break;
		case Log.ASSERT:
		...
		default:
			...
			if (settings.isLogger2File()) {
				settings.getLogger2File().d(finalTag, chunk);
			}
			break;
		}
	}
